...
Safari, however, doesn’t follow this behavior. There is a long‑standing [bug report in WebKit Backlog](https://bugs.webkit.org/show_bug.cgi?id=277290) where scrollable elements (like long code blocks or tables) do not receive keyboard focus filed by [Adrian Roselli](https://adrianroselli.com/2022/06/keyboard-only-scrolling-areas.html), making them inaccessible to keyboard users.## The Problem
...
Code blocks are usually implemented using `<pre>` elements. The `<pre>` element is the only non-interactive element that can become scrollable by default due to its content because it's designed to preserve formatting and not wrap content.
...
While this polyfill focuses on `<pre>` elements by default for performance reasons, it can be configure to also apply the same logic to other elements like tables, divs with `white-space: nowrap`, or any other scrollable container.
For example, if you have a class `scrollable` that prevents and element from wrapping, you can easily extend the polyfill to handle any element with that class.
