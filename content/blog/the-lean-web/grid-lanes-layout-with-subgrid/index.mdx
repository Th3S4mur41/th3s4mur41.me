...
In a grid, the table of contents shares the first row with the h1 or first paragraph. That row then grows to match the TOC height, which is usually much taller than a single heading.
...
The TOC height depends on content, so hard-coding row spans is guesswork.
...
At first glance, it feels like moving to lanes might break subgrid.
...
Unfortunately, combining `position: sticky` with anchor positioning is not currently possible.
