...
Thanks to [Myndex](https://bsky.app/profile/myndex.bsky.social) for following up on the previous update and highlighting an important point:
while early versions of APCA were indeed removed from the WCAG 3 Working Draft, discussions about better contrast algorithms are still ongoing in the working group.

You might also want to check out [Bridge-PCA](https://bridgepca.com/), a drop-in replacement for WCAG 2.
Bridge-PCA reports contrast not only as the APCA Lc value (lightness contrast), but also as a WCAG 2-compatible ratio display, but with a better algorithm.
